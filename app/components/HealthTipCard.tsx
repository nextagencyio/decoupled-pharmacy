...
interface HealthTipCardProps {
  item: DrupalHealthTip
}

export default function HealthTipCard({ item }: HealthTipCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="flex-1 p-6 flex flex-col justify-center">
        {(item as any).category && (
          <p className="text-sm text-primary-600 font-medium mb-1">{(item as any).category}</p>
        )}
