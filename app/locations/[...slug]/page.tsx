...
async function getLocation(path: string): Promise<DrupalLocation | null> {
  try {
    ...
    const { data } = await client.raw(GET_LOCATION_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Location Not Found | Pharmacy' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).stateProvince && (
                    <div>
                      <dt className="text-sm text-gray-500">State</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).stateProvince}</dd>
                    </div>
                  )}
                  {(item as any).postalCode && (
                    <div>
                      <dt className="text-sm text-gray-500">Postal Code</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).postalCode}</dd>
                    </div>
                  )}
                  {(item as any).phone && (
                    <div>
                  ...
                  {(item as any).fax && (
                    <div>
                      <dt className="text-sm text-gray-500">Fax Number</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).fax}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
